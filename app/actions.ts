...
async function requireOwnedBooth(boothId: string) {
  const user = await requireAdmin();
...
async function requireOwnedBoothPayment(paymentId: string) {
  const user = await requireAdmin();
...
async function requireOwnedBoothContact(contactId: string) {
  const user = await requireAdmin();
...
async function requireOwnedBoothVehicle(vehicleId: string) {
  const user = await requireAdmin();
...
async function requireOwnedBoothFile(fileId: string) {
  const user = await requireAdmin();
...
export async function createBoothFile(
  ...
  const booth = await requireOwnedBooth(boothId);
  assertFestivalMatch(booth.festivalId, festivalId);
  await prisma.boothFile.create({
    data: { boothId, name, url, isExternal, fileType: fileType || null },
  });
  revalidatePath(`/festivals/${festivalId}/booths`);
  revalidatePath(`/festivals/${festivalId}/documents`);
}
