...
export default function BoothForm({ token, initialContacts, initialVehicles, submitAction }: Props) {
    ...
    <div className="space-y-8">
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          ...
          <button type="button" onClick={addContact} className="text-sm text-violet-600 hover:text-violet-700 font-medium">
            + הוסף
          </button>
        </div>
          ...
          {contacts.map((c, i) => (
            <div key={i} className="bg-gray-50 rounded-xl p-3 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <input value={c.name} onChange={(e) => updateContact(i, "name", e.target.value)} placeholder="שם מלא *" className={inputCls} />
                <input value={c.role} onChange={(e) => updateContact(i, "role", e.target.value)} placeholder="תפקיד" className={inputCls} />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <input value={c.phone} onChange={(e) => updateContact(i, "phone", e.target.value)} placeholder="טלפון" className={inputCls} dir="ltr" />
                <input value={c.email} onChange={(e) => updateContact(i, "email", e.target.value)} type="email" placeholder="מייל" className={inputCls} dir="ltr" />
              </div>
            </div>
          ))}
        </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          ...
          <button type="button" onClick={addVehicle} className="text-sm text-violet-600 hover:text-violet-700 font-medium">
            + הוסף
          </button>
        </div>
          ...
          {vehicles.map((v, i) => (
            <div key={i} className="bg-gray-50 rounded-xl p-3 space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                <input value={v.plateNumber} onChange={(e) => updateVehicle(i, "plateNumber", e.target.value)} placeholder="מספר רישוי *" className={inputCls} dir="ltr" />
                <input value={v.vehicleType} onChange={(e) => updateVehicle(i, "vehicleType", e.target.value)} placeholder="סוג רכב" className={inputCls} />
                <input value={v.arrivalTime} onChange={(e) => updateVehicle(i, "arrivalTime", e.target.value)} placeholder="שעת הגעה" className={inputCls} />
              </div>
            </div>
          ))}
        </div>
